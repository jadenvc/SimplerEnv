...
Example:
    ...
    python simpler_env/simpler_env_inference.py --policy "ECoT" --task widowx_spoon_on_towel
    python simpler_env/simpler_env_inference.py --policy "vla" --task widowx_spoon_on_towel
    python simpler_env/simpler_env_inference.py --policy rt1 \
    ...
    python simpler_env/simpler_env_inference.py --policy octo-small \
...
parser.add_argument("--policy", default="rt1", choices=["rt1", "octo-base", "octo-small", "vla", "ECoT"])
parser.add_argument(
    "--ckpt-path",
    type=str,
...
if args.policy == "rt1":
    ...
    model = OpenVLAInference(model="vla", policy_setup=policy_setup)
else:
    from simpler_env.policies.openvla.vla_model import OpenVLAInference

    model = OpenVLAInference(model="ECoT", policy_setup=policy_setup)
